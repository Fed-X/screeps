...
export default class Transporter extends Creeper {
  run(): void {
    ...
    let spawns = _.filter(creeps, (c:any) => memory['creeps'][c.name]?.role == 'spawnTransporter')
    if (spawns.length == 0) {
      creepMemory.role = 'spawnTransporter'
    ...
    switch (creepMemory.task) {
      ...
      case 'filling': {
        if (!creepMemory.target) {
          ...
          let extensions = _.filter(creep.room.find(FIND_MY_STRUCTURES, { filter: { structureType: STRUCTURE_EXTENSION } }), function(struct:any){ return struct.store.getFreeCapacity(RESOURCE_ENERGY) > 0 })
          ...
          if (towers.length > 0 || spawns.length > 0 || extensions.length > 0) { resources = resources.concat(creep.room.find(FIND_MY_STRUCTURES, { filter: s => s.structureType == STRUCTURE_STORAGE && s.store.getUsedCapacity(RESOURCE_ENERGY) > 0 })) }
