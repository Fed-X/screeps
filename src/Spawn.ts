...
export default class Spawn extends ScreepObject {
  ...
  createCreep(): void {
    ...
    if (this.spawn.room.energyAvailable >= 250) {
      ...
      } else if (constructors.length < sources.length * 1.5) {  // Build some amount of constructors per source.. needs balancing
      ...
      } else if (upgraders.length < sources.length * 1.5) {     // Build some amount of upgraders per source.. needs balancing
